...
- they must be run in the sequence they were listed, starting by the preprocess directory. Second, one should run the scripts in the main data subdirectories "01_data". Third, one should run the scripts in the subdirector "02_analysis", in the order they are numbered.
...
- The [ERSST](https://www.ncei.noaa.gov/pub/data/cmb/ersst/v5/netcdf/) is downloaded automatically with an script and stored in the preprocessing directory. HadSST must be also downloaded to the preprocessing directory.
...
- ERA5 and ERA5-Land variables must be placed in data subdiretories "01_data" from 03 to 09, and the datasets are available from Copernicus Climate Data Store web portal https://cds.climate.copernicus.eu
